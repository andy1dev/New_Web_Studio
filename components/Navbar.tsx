    ...
    <>
      <motion.nav
        ...
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          isScrolled 
            ? 'py-3 bg-dark-950/90 backdrop-blur-xl border-b border-zinc-800/50' 
            : 'py-5 bg-transparent'
        }`}
      >
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="fixed inset-0 z-40 md:hidden bg-dark-950/98 backdrop-blur-xl pt-24 px-6"
          >
            <div className="flex flex-col gap-6">
              {navItems.map((item, index) => (
                <motion.button
                  ...
                  onClick={() => handleNavClick(item.href)}
                  className="text-2xl font-semibold text-left text-zinc-300 hover:text-white transition-colors"
                >
                  {item.label}
                </motion.button>
              ))}
